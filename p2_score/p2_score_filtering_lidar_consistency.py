...
def update_pred(ptc, pp_score, calib, preds, args, img_shape, confidences_per_class=None):
    ...
    # First, filter by P2 score (remove false positives that are too static)
    mask_p2 = np.array([
        filter_by_ppscore(
    ...
    # Then, filter by confidence per class (posterior matching with source ratio)
    if args.confidence_score_threshold > 0 and len(preds['name']) > 0:
        # Note: if confidence_per_class is not None, we force confidence_score_threshold to be > 0.
    ...
    if getattr(args, 'check_aum', None) is not None:
        # CURRENTLY NOT USED
        # Checks for the Area under the Margin (https://arxiv.org/abs/2001.10528) 
        # and filters out "confused" labels
...
def process_one_scene(idx, args, det_bboxes, confidences_per_class=None):
...
def process_batch_scene(idx_list, args, det_bboxes, confidences_per_class=None):
    return [
        process_one_scene(idx, args, det_bboxes, confidences_per_class)
        for idx in idx_list
    ]
...
def main(args):
    ...
    # compute the confidence scores per class to use for posterior filtering
    ...
    if getattr(args.det_filtering, "posterior_regularization", []):
        ...
        assert args.det_filtering.confidence_score_threshold > 0.  # ensure this is always larger than 0.
        ...
        for name in confidences:
            source_domain_ratio = getattr(posterior_regularization, name.lower(), None)
            print("Posterior source domain ratio for", name.lower(), ":", source_domain_ratio)
            if source_domain_ratio is None:
                num_box_per_class = 0
            else:
                source_domain_ratio *= args.det_filtering.posterior_regularization.alpha  # multiply by confidence constant
                num_box_per_class = int(source_domain_ratio * len(det_bboxes))  # int(len(confidences[name]) * (1 - percentile))
            ...
            print("Posterior regularization confidence:", confidences_per_class[name])
    ...
    for idx_sublist in np.array_split(idx_list, args.n_processes):
        det_bboxes_new.append(process_batch_scene.remote(
            idx_sublist, args, det_bboxes_id, confidences_per_class))

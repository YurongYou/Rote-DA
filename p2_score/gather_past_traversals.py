...
def main(args):
    ...
    idx_list = np.array(idx_list)[args.start_idx:args.end_idx]
    ...
    for origin_idx in tqdm(idx_list):
        ...
        combined_lidar = {}
        combined_lidar_path = osp.join(args.traversal_ptc_save_root, f"{origin_idx:06d}.pkl")
        if not osp.exists(combined_lidar_path):
        ...
        trans_mat_path = osp.join(args.trans_mat_save_root, f"{origin_idx:06d}.npy")
        if not osp.exists(trans_mat_path):
            trans_mat = get_relative_pose(
                fixed_l2e=first_l2e, fixed_ego=first_pose,
                query_l2e=origin_l2e, query_ego=origin_pose,
                KITTI2NU=_KITTI2NU_nusc if args.nusc else _KITTI2NU_lyft)
            np.save(trans_mat_path, trans_mat)

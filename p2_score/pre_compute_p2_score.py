...
def compute_and_save_p2_score_lyft(args):
    ...
    oxts_path = osp.join(args.data_paths.scan_path, "../oxts")
    l2e_path = osp.join(args.data_paths.scan_path, "../l2e")
    ...
    if args.data_paths.index_file_train is not None:
        idx_list = [int(x) for x in open(args.data_paths.index_file_train).readlines()]
    else:
    ...
    for origin_idx in tqdm(valid_train_idx_list):
        if osp.exists(osp.join(args.data_paths.p2score_path, f"{origin_idx:06d}")):
            continue
        ...
        for seq_id, indices in valid_idx[origin_idx][2]:
            ...
            for frame in indices:
                _ptc = load_velo_scan(
                    osp.join(args.data_paths.scan_path,
...
def compute_and_save_p2_score_ithaca365(args):
    ...
    # for sample in tqdm(dataset.sample_with_accurate_localization[args.start_idx:args.end_idx]):
        ...
        persistency_score = dataset.get_point_persistency_score(sd_token, num_histories=20, ranges=(0, 70))
        np.save(osp.join(args.data_paths.p2score_path,
...
def main(args: DictConfig):
    ...
    # make the directory for saving the P2 scores
    os.makedirs(args.data_paths.p2score_path, exist_ok=True)

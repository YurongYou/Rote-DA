...
from train_utils.train_st_utils import train_model_st, train_model
...
def parse_config():
    ...
    parser.add_argument('--empty_cache_every', type=int,
                        default=-1, help='empty cuda cache every x iter')
...
def main():
    ...
    # total iterations per epoch is set to the number of iterations to go through the target dataset if possible
    if cfg.get('SELF_TRAIN', None):
    ...
    if cfg.LOCAL_RANK == 0 and args.wandb_project is not None:
        wandb.config.update({"total_step": total_iters_each_epoch * args.epochs})
        # wandb.config.update(cfg)
        wandb.watch(model)
    ...
    for dset in [source_set, target_set]:
        if (dset is not None) and hasattr(dset, 'use_shared_memory') and dset.use_shared_memory:
            dset.clean_shared_memory()
    ...
    if target_set is None:
        testset_cfg = cfg.DATA_CONFIG
        testset_classes = cfg.CLASS_NAMES
    else:
        testset_cfg = cfg.DATA_CONFIG_TAR
        testset_classes = cfg.DATA_CONFIG_TAR.CLASS_NAMES

    test_set, test_loader, sampler = build_dataloader(
        dataset_cfg=testset_cfg,
        class_names=testset_classes,
        batch_size=args.batch_size,

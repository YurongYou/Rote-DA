...
class CosineAnneal(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, lr_min):
        # self.lr_max = lr_max
        # self.moms = moms
        # low_lr = lr_max / self.div_factor
        lr_phases = ((0, partial(annealing_cos, lr_max, lr_min)),)
        mom_phases = ((0, partial(annealing_cos, *moms[::-1])),)
        fai_optimizer.lr, fai_optimizer.mom = lr_max, moms[0]
        super().__init__(fai_optimizer, total_step, lr_phases, mom_phases)

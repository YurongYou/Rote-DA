...
# to keep track intermediate pseudolabels 
# this dictionary will be used to update PSEUDO_LABELS
NEW_PSEUDO_LABELS = {}
...
def check_already_exist_pseudo_label(ps_label_dir, start_epoch):
    ...
    if start_epoch == 0 and cfg.SELF_TRAIN.get('INIT_PS', None):
        if os.path.exists(cfg.SELF_TRAIN.INIT_PS):
            ...
            class_dist = [PSEUDO_LABELS[p]['gt_boxes'][:, 7] for p in PSEUDO_LABELS]
            class_dist = Counter(np.concatenate(class_dist).tolist())

            return cfg.SELF_TRAIN.INIT_PS, class_dist

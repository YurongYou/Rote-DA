...
class PointHeadBox(PointHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            if self.p2_supervision is not None and self.p2_supervision.get("test_p2_filter", False):
                p2_score = batch_dict['p2_score']
                mask = p2_score < 0.5
                point_cls_preds = point_cls_preds[mask]
                point_box_preds = point_box_preds[mask]

                batch_dict['batch_cls_preds'] = point_cls_preds
                batch_dict['batch_box_preds'] = point_box_preds
                batch_dict['batch_index'] = batch_dict['point_coords'][mask, 0]

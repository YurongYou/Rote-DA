...
class WaymoDataset(DatasetTemplate):
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(),
                  has_label=True, sampled_interval=1, reverse=False,
                  total_part=1, part=0):
        ...
        if reverse:
            sample_sequence_file_list = sample_sequence_file_list[::-1]
        if args.total_part > 1:
            seq_idx_list = np.arange(len(sample_sequence_file_list), dtype=int)
            seq_idx_list = np.array_split(seq_idx_list, total_part)[part]
            sample_sequence_file_list = [
                sample_sequence_file_list[idx] for idx in seq_idx_list]
        ...
        sequence_infos = [waymo_utils.process_single_sequence.remote(
            seq, save_path=save_path, sampled_interval=sampled_interval, has_label=has_label) for seq in sample_sequence_file_list]
        sequence_infos = ray.get(sequence_infos)
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
                       ...
                       workers=16, reverse=False, total_part=1, part=0, skip_info=False):
    ...
    waymo_infos_train = dataset.get_infos(
        ...
        sampled_interval=1, reverse=reverse, total_part=total_part, part=part
    )
    if skip_info:
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip_info', action='store_true')

...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist',
                   'van', 'person_sitting', 'truck', 'dynamic']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = 0
        elif (current_cls_name == "Car".lower() and
              (("Van".lower() == gt_name)
               or ("Truck".lower() == gt_name)
               )):
            valid_class = 0
        else:
            valid_class = -1
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                threshold_all[m, l, k, :len(thresholds)] = thresholds
...
def do_eval(gt_annos,
    ...
    if PR_detail_dict is not None:
        ...
        PR_detail_dict['bev_threshold'] = ret['threshold']
    ...
    if PR_detail_dict is not None:
        ...
        PR_detail_dict['3d_threshold'] = ret['threshold']
...
def filter_det_range(dets, close, far):
    dets = deepcopy(dets)
    valid_idx = (dets['location'][:, 2] > close) * \
        (dets['location'][:, 2] <= far)
    for k in dets:
...
def get_range_eval_result(gt_annos,
    ...
    for range_s, range_e in range_pairs:
        ...
        _, _, _, _, _, mAPbev_R40, mAP3d_R40, _ = do_eval(
            gt_annos_range, dt_annos_range, current_classes, min_overlaps, compute_aos, difficultys=[
                3],

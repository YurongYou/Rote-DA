...
class KittiDataset(DatasetTemplate):
    ...
    def get_history_raw(self, idx):
        ...
        if self.history_cache_dir is not None and \
                osp.exists(osp.join(self.history_cache_dir, f"{idx}.pth")):
            try:
        ...
        if history_scans is None:
            ...
            history_scans = [transform_points(
                x, np.linalg.inv(trans_mat)) for x in history_scans]
    ...
    def get_history(self, idx):
        ...
        if self.history_cache_dir is not None and \
            ...
            if self.load_history.FORWARD_ONLY:
                history_scans = [x[x[:, 0] > 0, :] for x in history_scans]
            history_coordinates = [ME.utils.sparse_quantize(
                x / self.load_history.VOXEL_SIZE) for x in history_scans]
            if self.history_cache_dir is not None:
                torch.save(
                    history_coordinates,
                    osp.join(self.history_cache_dir, f"{idx}.pth")
        ...
        if self.load_history.LIMIT_NUM > 0 and len(history_coordinates) > self.load_history.LIMIT_NUM:
            if self.training:
                _scan_num_choice = np.random.choice(
        ...
        history_features = [torch.ones((len(x), 1))
                            for x in history_coordinates]
    ...
    def get_p2_score(self, idx):
        ...
        return np.load(os.path.join(self.load_p2_score, f"{idx}.npy")).astype(np.float32)
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                annotations['location'] = np.concatenate([obj.loc.reshape(
                    1, 3) for obj in obj_list], axis=0) if len(obj_list) > 0 else np.array([]).reshape(0, 3)
    ...
    def evaluation(self, det_annos, class_names, range_eval=True, ranges=(0, 30, 50, 80), **kwargs):
    ...
    def __getitem__(self, index):
        ...
        if "p2_score" in get_item_list and self.training:
            input_dict["p2_score"] = self.get_p2_score(
                sample_idx)
        ...
        if "history_scans" in get_item_list:
            if self.load_history.get("HISTORY_AUG", False):
                ...
                input_dict["history_coordinates"], input_dict["history_features"] = self.get_history(
                    sample_idx)

        if "background_sample" in get_item_list and self.training:
            input_dict["background_sample"] = self.get_background_sample(
                sample_idx)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        dataset.update_groundtruth_database(
            source_info_path=sys.argv[4],
            det_info_path=sys.argv[5],
            split='train')
